...
exports.getRestaurants = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError('Fetching restaurants failed, please try again later.', 500)
    return next(error)
  }
  ...
  res.json({
    restaurants: restaurants.map(restaurant => restaurant.toObject({ getters: true })),
  })
}
...
exports.getRestaurantMenu = async (req, res, next) => {
  ...
  let restaurant
  try {
    restaurant = await Restaurant.findById(restaurantId).populate('products')
  } catch (err) {
    const error = new HttpError('Fetching restaurant failed, please try again later.', 500)
    console.log({ err })
    return next(error)
  }
